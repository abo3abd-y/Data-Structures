...
bool allIsBalance(string str) { // do not forget that when you are comparing a character in a string, use single quotes and not double quotes
    Node *nd;
    nd = nullptr;

    for(std::string::size_type i = 0; i < str.length(); i ++) { 
        if(str[i] == '(' || str[i] == '{' || str[i] == '[') { // first change
            push(nd, 1); 

        }else if(str[i] == ')' || str[i] == '}' || str[i] == ']') { // that this is literally the only changes that you have to make from the new function as you can see here
            if(isEmpty(nd)) {return false;}
            pop(nd);
        }
    }
...
int main () {
    ...
    cout << allIsBalance(st) << endl; // should return true as you can see here
