...
void Delete (int position) {
    if(head) {
        if(position ==1) {
            ...
            p -> next = head -> next;
            delete head; // remember delete just means deallocate the current pointed memory
            head = p -> next;
        }else if(position > 0 && position <= length(head)) {
            Node *p = head;
            Node *prev = nullptr;
...
int main () { 
    ...
    Delete(7);
    displayCircular(head); // it works perfect as intended as you can see here; we reviewed the edges casee and any other cases and it worked as you can see
