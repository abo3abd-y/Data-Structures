...
bool isLoop (Node *src) { //it is like a circular race; the faster car is guranteed to meet the slower car at some point in the future assuming there are unlimited laps.
    Node *p = src;
    Node *q = src;
...
int main () { 
    ...
    Node *another_first = another_create(a, 6);

    Node *t1, *t2;

    t1 = another_first -> next -> next;
    t2 = t1 -> next -> next -> next;
    t2 -> next = t1;

    cout << endl;
    cout << isLoop(another_first) << endl; //works perfectly
