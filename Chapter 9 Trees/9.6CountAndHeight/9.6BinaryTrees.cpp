#include <iostream>
#include "9.6BinaryTree.h"
#include "9.6StackForTraversal.h"
...
void binaryTree::levelOrder() {
    ...
    while(!q.isEmpty()) {
        temp = q.dequeue(); // again the tree node needs modification because it does not really deallocate the memory properly
        cout << temp -> data << " ";
        if(temp -> left) {
            q.enqueue(temp -> left);
        }
...
int binaryTree::nodeCounter(treeNode *binaryNode) { // we almost always use postorder traversal when doing some analysis such as counting
    int x,y;
    if(binaryNode) {

        x = nodeCounter(binaryNode-> left);
        y = nodeCounter(binaryNode -> right);
...
int main () {
    ...
    b.levelOrder(); // works perfectly as you can see here

    cout << "There are " << b.nodeCounter(b.return_root()) <<" nodes in the binary tree." << endl; // as you can see this works perfectly
